...
template <class TPixel = float, unsigned int VImageDimension = 2>
...
int itkDiffeomorphicDemonsRegistrationWithMaskFilterTest(int argc, char **argv)
{
  ...
  typedef itk::Image<itk::Vector<PixelType, dims>, dims> TDisplacementField;
  ...
  typedef itk::DiffeomorphicDemonsRegistrationWithMaskFilter<
            ImageType, ImageType,
            TDisplacementField>  RegistrationFilterType;
...
//  filter->Print(std::cout,6);
  ...
  CommandIterationUpdate<float,3>::Pointer
  ...
  typedef itk::WarpImageFilter<ImageType, ImageType, TDisplacementField> WarperType;
  ...
  if( argc > 6)
  {
    typedef itk::ImageFileWriter<TDisplacementField> DisplacementFieldWriterType;
    DisplacementFieldWriterType::Pointer displacementWriter = DisplacementFieldWriterType::New();
    displacementWriter->SetInput( filter->GetOutput() );
    displacementWriter->SetFileName( argv[6] );
